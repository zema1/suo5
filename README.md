...
Suo5 针对 `auto` 模式（默认）做了大量的优化，大多数时候你无需复杂参数配置，仅使用 `-t` 指定服务端 URL，即可自动确认目标支持的通信模式并在本地启动代理服务：
...
GUI 版基于 [Wails](https://github.com/wailsapp/wails) 构建，支持 Windows 和 macOS 和 Linux 三大主流系统。针对日常使用习惯做了大量改进优化，界面功能简洁易懂，推荐大多数用户使用此版本。
...
全双工模式下单个 HTTP 请求即可实现双向通信，效率最高。半双工是在不支持全双工的环境（如某些反向代理配置）下的降级方案，使用长连接下行+短连接上行实现通信。
