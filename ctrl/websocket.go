	...
	go func() {
		...
		for {
			...
			case data, ok := <-s.writeChan:
				...
				if s.config.RequestInterval != 0 {
					log.Debugf("sleep %d ms", s.config.RequestInterval)
	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				s.needPing = true
				log.Debugf("send websocket ping message")
				err := s.wsConn.WriteMessage(websocket.PingMessage, nil)
				s.writeMu.Unlock()
				if err != nil {
					log.Errorf("write ping message failed: %v", err)
					return
				}
			}
		}
