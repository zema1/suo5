...
func Run(ctx context.Context, config *Suo5Config) error {
	...
	if config.Transport == TransportHTTP {
		...
		for i := 0; i < config.MaxRetry; i++ {
			log.Infof("try to connect to %s, %d/%d", config.Target, i, config.MaxRetry)
		...
		if multiplex == nil {
			return fmt.Errorf("failed to connect to target %s", config.Target)
		}
	} else if config.Transport == TransportHTTPMultiplex {
		if config.RequestInterval == 0 {
			log.Infof("change request interval to 300ms")
			config.RequestInterval = 300
		}
